...
static int my_savejpg = 0;//0 不保存 1 保存
static int my_motion = 1;//0 不检测 1 检测
...
int input_init(input_parameter *param) {
  ...
  while(1) {
    ...
    static struct option long_options[] = \
    {
	  ...
	  {"M", required_argument, 0, 0},
      {"Motion", required_argument, 0, 0},      //运动检测
	  {"s", required_argument, 0, 0},
      {"savejpg", required_argument, 0, 0},     //保存jpg文件
      {"N", required_argument, 0, 0},
      {"Noise", required_argument, 0, 0}, // 噪声水平
      {"t", required_argument, 0, 0},
      {"threshold", required_argument, 0, 0}, //变化阈值
      {0, 0, 0, 0}
    };
    ...
    switch (option_index) {
      ...
      case 23:
        DBG("case 22,23\n");
        param_init( atoi(optarg), -1 );
        break;
	  /* t, threshold */
      case 24:
      case 25:
        DBG("case 24,25\n");
        param_init( -1, atoi(optarg) );
        break;
      default:
        DBG("default case\n");
        help();
...
void help(void) {
  ...
  fprintf(stderr, " [-f | --fps ]..........: frames per second\n" \
                  ...
                  " [-M | --Motion ].......: switch the MOTION \"on\", \"off\", default \"on\"\n" \
                  " [-s | --savejpg ]......: switch the save to jpeg \"on\", \"off\", default \"off\"\n" \
                  " [-N | --Noise ]........: Noise value  \"0 - 200\", default \"20\"\n" \
                  " [-t | --threshold ]....: threshold value  \"0 - 10000\", default \"100\"\n" \
                  " ---------------------------------------------------------------\n\n");
}
int save2jpg( unsigned char *pic, int size )
{
	printf("------save2jpg-------\n");
    ...
    time(&tt);
    t = localtime(&tt);
    snprintf(name, 50, "%d%d%d%d%d%d.jpg",
...
void *cam_thread( void *arg ) {
  ...
  unsigned char *tmp_rgb24;
  int timecount = 5;
  tmp_rgb24 = (unsigned char*)calloc(1,3 * videoIn->width * videoIn->height);
  while( !pglobal->stop ) {
    ...
    if (videoIn->formatIn == V4L2_PIX_FMT_YUYV) {
	  ...
	  if( my_motion )
	  {
		  int motion = motion_check( videoIn->framebuffer, tmp_rgb24, videoIn->width, videoIn->height );
		  if( 0 == timecount && 0xaa == motion )
		  {
			timecount = 5;//防止同一帧存储多次
		  }
		  pglobal->size = compress_rgb24_to_jpeg(tmp_rgb24, pglobal->buf, videoIn->width , videoIn->height, gquality);
		  if( timecount == 5 && my_savejpg ) {
			save2jpg( pglobal->buf, pglobal->size );
		  }
