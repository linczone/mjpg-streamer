...
int compress_rgb24_to_jpeg(unsigned char *in, unsigned char *out, unsigned int width, unsigned int height, int quality) {
  struct jpeg_compress_struct cinfo;
  struct jpeg_error_mgr jerr;
  JSAMPROW row_pointer[1];
  unsigned char *line_buffer, *rgb24;
  int z;
  static int written;
  ...
  jpeg_create_compress (&cinfo);
  /* jpeg_stdio_dest (&cinfo, file); */
  dest_buffer(&cinfo, out, height*width * 3, &written);
  ...
  {
	row_pointer[0] = &rgb24[cinfo.next_scanline * z];
